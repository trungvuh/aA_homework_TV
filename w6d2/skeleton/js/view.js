...
View.prototype.exercise5 = function () {
  ...
  //your code here!
  $('.square').on("click", el => {
    const $check = $(el.currentTarget);
    alert($check.attr("data-pos"));
  });
};
...
View.prototype.exercise6 = function () {
  ...
  $(".square").each( (idx, el) => {
    let $check = $(el);
    $check.css("background-color", _randomColorString());
  });
};
...
View.prototype.exercise7 = function(){
  ...
  $("#easel").on("mouseenter", ".square", el => {
    const $check = $(el.currentTarget);
    console.log($check.css("background-color"));
  });
};
